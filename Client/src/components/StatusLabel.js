...
function StatusLabel(props) {
    return (
        <div className={'StatusLabel type' + props.type}>
            ...
            {props.type === 1 && <span className='status-name'>Đang chờ xác nhận</span>}
            {props.type === 2 && <span className='status-name'>Đang vận chuyển</span>}
            {props.type === 3 && <span className='status-name'>Đã giao hàng</span>}
